...
import {
  ...
  dismissNewQuizAlert,
  getUserHiddenQuizIds,
  hideQuizForUser,
  unhideQuizForUser,
  unhideAllQuizzesForUser,
} from "../utils/storage";
import { fetchQuizAttemptsFromFirestore } from "../lib/quizService";
...
export const HomeScreen: React.FC<HomeScreenProps> = ({
  ...
  // Hidden/dismissed quizzes state for student personal area
  const [hiddenQuizIds, setHiddenQuizIds] = useState<number[]>(() =>
    getUserHiddenQuizIds(currentUserEmail)
  );
  ...
  useEffect(() => {
    ...
    const loadAttempts = async () => {
      // Local immediate load
      const localAttempts = getStoredQuizAttempts(currentUserEmail);
      if (isMounted) setAttempts(localAttempts);

      try {
        const firestoreAttempts = await fetchQuizAttemptsFromFirestore(currentUserEmail);
        if (isMounted && firestoreAttempts.length > 0) {
          setAttempts(firestoreAttempts);
        }
      } catch (err) {
        console.warn("Could not fetch remote attempts, using local cache:", err);
      }
    };

    loadAttempts();
    setDismissedQuizIds(getDismissedNewQuizIds(currentUserEmail));
    setHiddenQuizIds(getUserHiddenQuizIds(currentUserEmail));
  ...
  // Filter quizzes according to RBAC (Admins see everything; students see public ones not hidden in their area)
  const accessibleQuizzes = isAdmin
    ? quizzes
    : quizzes.filter((q) => q.isPublic !== false && !hiddenQuizIds.includes(q.id));

  // Quizzes hidden only for this student
  const studentHiddenQuizzes = !isAdmin
    ? quizzes.filter((q) => q.isPublic !== false && hiddenQuizIds.includes(q.id))
    : [];
  ...
  return (
    <div id="home-screen" className="w-full max-w-4xl mx-auto px-3 sm:px-4 py-4 sm:py-6 pb-28 space-y-5 sm:space-y-6 overflow-hidden">
      ...
      <div className="space-y-3 w-full max-w-full">
                  ...
                  <div className="flex items-start justify-between gap-2.5">
                    ...
                    {isAdmin ? (
                      ...
                      <div className="flex items-center gap-0.5 shrink-0">
                        <button
                          type="button"
                          id={`delete-quiz-student-${quiz.id}`}
                          onClick={(e) => {
                            e.stopPropagation();
                            setQuizToDelete(quiz);
                          }}
                          className="p-1.5 text-slate-500 hover:text-rose-400 hover:bg-slate-800/80 rounded-xl transition-colors cursor-pointer"
                          title="Excluir da minha área"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
        ...
        {/* Student Hidden Quizzes Restore Section */}
        {!isAdmin && studentHiddenQuizzes.length > 0 && (
          <div
            id="student-hidden-quizzes-section"
            className="p-4 rounded-3xl bg-slate-900/60 border border-slate-800/80 space-y-3 animate-in fade-in duration-200"
          >
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-300">
                    Questionários Removidos da Sua Área ({studentHiddenQuizzes.length})
                  </h4>
                  <p className="text-[11px] text-slate-500">
                    Ocultados apenas na sua conta; permanecem ativos para outros usuários.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {showHiddenList ? "Ocultar lista" : "Visualizar"}
                </button>
                <button
                  type="button"
                  onClick={() => {
                    if (currentUserEmail) {
                      unhideAllQuizzesForUser(currentUserEmail);
                      setHiddenQuizIds([]);
                    }
                  }}
                  className="px-3 py-1 rounded-xl bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 text-xs font-bold transition-colors cursor-pointer"
                >
                  Restaurar Todos
                </button>
              </div>
            ...
            {showHiddenList && (
              <div className="pt-2 space-y-2 border-t border-slate-800/80">
                {studentHiddenQuizzes.map((hq) => (
                  <div
                    key={hq.id}
                    className="flex items-center justify-between p-3 rounded-2xl bg-slate-950/70 border border-slate-800/60 gap-3"
                  >
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-bold text-slate-200 truncate">{hq.title}</p>
                      <p className="text-[10px] text-slate-500">{hq.category} • {hq.questionCount || hq.questions?.length || 0} questões</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => {
                        if (currentUserEmail) {
                          unhideQuizForUser(currentUserEmail, hq.id);
                          setHiddenQuizIds((prev) => prev.filter((id) => id !== hq.id));
                        }
                      }}
                      className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-indigo-300 hover:text-white text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer shrink-0"
                    >
          ...
          <div className="w-full max-w-md p-5 sm:p-6 rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl space-y-4 animate-in fade-in zoom-in duration-150">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <h4 className="text-base sm:text-lg font-bold text-white leading-tight">
                  {isAdmin ? "Excluir da Base de Dados?" : "Excluir da Sua Área?"}
                </h4>
                <span
                ...
                >
                  {isAdmin ? "Ação de Administrador • Global" : "Área do Estudante • Individual"}
                </span>
              </div>
            </div>
            ...
            <div className="p-3.5 rounded-2xl bg-slate-950/80 border border-slate-800 text-xs text-slate-300 space-y-2">
              <p className="font-semibold text-white">
                Questionário: <span className="text-indigo-300 font-bold">&ldquo;{quizToDelete.title}&rdquo;</span>
              </p>
              {isAdmin ? (
                <p className="text-slate-400 leading-relaxed">
                  Esta ação apagará permanentemente o questionário da base de dados Firebase. Ele será removido para <strong>todos os usuários e estudantes</strong>.
                </p>
              ) : (
                <p className="text-slate-400 leading-relaxed">
                  Este questionário será excluído <strong>apenas da sua visualização e área pessoal</strong>. Ele continuará salvo na base de dados e <strong>disponível para todos os outros estudantes e administradores</strong> que não executaram esta ação.
                </p>
              )}
            </div>
              ...
              <button
                ...
                onClick={() => {
                  if (isAdmin) {
                    ...
                    if (currentUserEmail) {
                      hideQuizForUser(currentUserEmail, quizToDelete.id);
                      setHiddenQuizIds((prev) => Array.from(new Set([...prev, quizToDelete.id])));
                    }
                    onDeleteQuiz(quizToDelete.id);
                  }
                  setQuizToDelete(null);
                }}
                className={`px-4 py-2.5 rounded-xl text-white text-xs font-bold transition-all active:scale-95 cursor-pointer shadow-lg ${
                  isAdmin
                    ? "bg-rose-600 hover:bg-rose-500 shadow-rose-900/30"
                    : "bg-amber-600 hover:bg-amber-500 shadow-amber-900/30"
                }`}
              >
                {isAdmin ? "Excluir Definitivamente" : "Excluir da Minha Área"}
              </button>
            </div>
          </div>
        </div>
